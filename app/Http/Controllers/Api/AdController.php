...
class AdController extends Controller
{
    public function index()
    {
        ...
        if (count($ads) > 0) {
            if ($ads->total() > $ads->perPage()) {/// لو فيه داتا اكتر من المعروضة اعمل pagination 
                $data = [
    ...
    public function search(Request $request)
    {
        ...
        //when بتاخد condition وبتعمل call back function 
    ...
    public function create(AdRequest $request)
    {
        ...
        $data['user_id'] = $request->user()->id;//sanctum handle this 
    ...
    public function update(AdRequest $request, $adId)
    {
        ...
        ///adId ال id بتاع ال row في جدول ال ads
        if ($ad->user_id != $request->user()->id) {////sanctum handle this  user id from token 
    ...
    public function delete(Request $request, $adId)
    {
       ...
       // if(!$ad) return ApiResponse::sendResponse(200, 'no match record in ads table with this id', []);
